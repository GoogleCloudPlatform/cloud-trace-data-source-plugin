...
var (
	...
	projectID = flag.String("project-id", "",
		"Project ID of the Google Cloud Trace scoping project to query. Queries sent to this project will union results from all projects within the scope.")
...
func getOAuth2Token(credentialsFile string) (string, error) {
	...
	if credentialsFile == "" {
		...
		token, err = google.DefaultTokenSource(ctx, "https://www.googleapis.com/auth/cloud-platform")
		if err != nil {
			return "", err
		}
	} else {
		...
		token, err = google.JWTAccessTokenSourceWithScope(jsonKey, "https://www.googleapis.com/auth/cloud-platform")
		if err != nil {
...
func buildUpdateDataSourceRequest(dataSource grafana.DataSource, token string, projectId string) (*grafana.DataSource, error) {
	if dataSource.Type != "googlecloud-trace-datasource" {
		return nil, errors.New("datasource type is not googlecloud-trace-datasource")
	}
